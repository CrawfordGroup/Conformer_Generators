...
while [ $conformer_count -le $snapshots ]; do
    ...
    sed -i '' 's/MOLECULE_NAME_CONFORMER_NUMBER/'${molecule_name}_cmpd_${conformer_count}_Opt${spectroscopy}'/g' $cwd/${molecule_name}_MD/cmpd_$conformer_count/G09_sub_SLURM.sh
    ...
    sed -i '' 's/MOLECULE_NAME_CONFORMER_NUMBER/'${molecule_name}_cmpd_${conformer_count}_Opt${spectroscopy}'/g' $cwd/${molecule_name}_MD/cmpd_$conformer_count/input.dat
...
for index in {1..${snapshots}}; do
    ...
    #atom_number+=($(awk '{print $1}' cmpd_${index}))
    #atom_sym+=($(awk '{print $2}' cmpd_${index}))
    #X+=($(awk '{print $3}' cmpd_${index}))
    #Y+=($(awk '{print $4}' cmpd_${index}))
    #Z+=($(awk '{print $5}' cmpd_${index}))
    #atom_type+=($(awk '{print $6}' cmpd_${index}))
    ...
    #echo "Length of Box (X-Direction): $box_X"
    box_Y=$(awk 'NR==2{print $2}' cmpd_${index})
    #echo "Length of Box (Y-Direction): $box_Y"
    box_Z=$(awk 'NR==2{print $3}' cmpd_${index})
    #echo "Length of Box (Z-Direction): $box_Z"    
    ...
    for a in {1..${#solvent_number[@]}}; do
        #echo "${solvent_number[$a]}"
        if (( $(bc -l <<< "$((${solvent_X[a]} - ${avg_X})) > $((${box_X}/2))") )); then
            #echo "X Before: ${solvent_X[$a]}"
            solvent_X[$a]=$(bc -l <<< "${solvent_X[$a]} - ${box_X}")
            #echo "X After: ${solvent_X[$a]}"
        elif (( $(bc -l <<< "$((${solvent_X[a]} - ${avg_X})) < $((-${box_X}/2))") )); then
            #echo "X Before: ${solvent_X[$a]}"
            solvent_X[$a]=$(bc -l <<< "${solvent_X[$a]} + ${box_X}")
            #echo "X After: ${solvent_X[$a]}"
        fi
        if (( $(bc -l <<< "$((${solvent_Y[a]} - ${avg_Y})) > $((${box_Y}/2))") )); then
            #echo "Y Before: ${solvent_Y[$a]}"
            solvent_Y[$a]=$(bc -l <<< "${solvent_Y[$a]} - ${box_Y}")
            #echo "Y After: ${solvent_Y[$a]}"
        elif (( $(bc -l <<< "$((${solvent_Y[a]} - ${avg_Y})) < $((-${box_Y}/2))") )); then
            #echo "Y Before: ${solvent_Y[$a]}"
            solvent_Y[$a]=$(bc -l <<< "${solvent_Y[$a]} + ${box_Y}")
            #echo "Y After: ${solvent_Y[$a]}"
        fi
        if (( $(bc -l <<< "$((${solvent_Z[a]} - ${avg_Z})) > $((${box_Z}/2))") )); then
            #echo "Z Before: ${solvent_Z[$a]}"
            solvent_Z[$a]=$(bc -l <<< "${solvent_Z[$a]} - ${box_Z}")
            #echo "Z After: ${solvent_Z[$a]}"
        elif (( $(bc -l <<< "$((${solvent_Z[a]} - ${avg_Z})) < $((-${box_Z}/2))") )); then
            #echo "Z Before: ${solvent_Z[$a]}"
            solvent_Z[$a]=$(bc -l <<< "${solvent_Z[$a]} + ${box_Z}")
            #echo "Z After: ${solvent_Z[$a]}"
        fi
    ...
    #visual_number=(${solute_number[@]})
    #visual_sym=(${solute_sym[@]})
    #visual_X=(${solute_X[@]})
    #visual_Y=(${solute_Y[@]})
    #visual_Z=(${solute_Z[@]})

    #visual_number+=(${solvent_number[@]})
    #visual_sym+=(${solvent_sym[@]})
    #visual_X+=(${solvent_X[@]})
    #visual_Y+=(${solvent_Y[@]})
    #visual_Z+=(${solvent_Z[@]})
    
    #for i in {1..${#visual_number[@]}}; do
    #    printf "%-2s \t %-10s \t %-10s \t %-10s\n" "${visual_sym[i]}" "${visual_X[i]}" "${visual_Y[i]}" "${visual_Z[i]}" >> visual_${index}
    #done

    # Compute average coordinate of solvent molecule.
    #avg_X_sol=0
    #avg_Y_sol=0
    #avg_Z_sol=0
    #for m in {1..${#solvent_number[@]}}; do
    #    let avg_X_sol+=$(bc -l <<< "scale=6; ${solvent_X[m]}/${#solvent_number[@]}")
    #    let avg_Y_sol+=$(bc -l <<< "scale=6; ${solvent_Y[m]}/${#solvent_number[@]}")
    #    let avg_Z_sol+=$(bc -l <<< "scale=6; ${solvent_Z[m]}/${#solvent_number[@]}")
    #done

    #echo "Avg. Solvent Coordinates:"
    #echo "${avg_X_sol}"
    #echo "${avg_Y_sol}"
    #echo "${avg_Z_sol}"
    ...
    # Determines solvent molecules distance from heavy atoms and appends.
    ...
    while [ ${solvent_complete} = false ]; do
        ...
        for p in {1..${#final_number[@]}}; do
            ...
            columns=$(awk -v ln="${ln_num}" '$1 == ln {print NF}' cmpd_${index})
            if [ ${columns} -eq 6 ]; then
                ...
                connect_1+=($(awk -v ln="${ln_num}" '$1 == ln {print $7}' cmpd_${index}))
                connect_2+=(0)
                connect_3+=(0)
                connect_4+=(0)
            elif [ ${columns} -eq 8 ]; then
                connect_1+=($(awk -v ln="${ln_num}" '$1 == ln {print $7}' cmpd_${index}))
                connect_2+=($(awk -v ln="${ln_num}" '$1 == ln {print $8}' cmpd_${index}))
                connect_3+=(0)
                connect_4+=(0)
            elif [ ${columns} -eq 9 ]; then
                connect_1+=($(awk -v ln="${ln_num}" '$1 == ln {print $7}' cmpd_${index}))
                connect_2+=($(awk -v ln="${ln_num}" '$1 == ln {print $8}' cmpd_${index}))
                connect_3+=($(awk -v ln="${ln_num}" '$1 == ln {print $9}' cmpd_${index}))
                connect_4+=(0)
            elif [ ${columns} -eq 10 ]; then
                connect_1+=($(awk -v ln="${ln_num}" '$1 == ln {print $7}' cmpd_${index}))
                connect_2+=($(awk -v ln="${ln_num}" '$1 == ln {print $8}' cmpd_${index}))
                connect_3+=($(awk -v ln="${ln_num}" '$1 == ln {print $9}' cmpd_${index}))
                connect_4+=($(awk -v ln="${ln_num}" '$1 == ln {print $10}' cmpd_${index}))
            fi
            col+=(${columns})
        done
        ...
        for s in {1..${#new_number[@]}}; do
            ...
            #final_number+=($(awk -v ln="${ln_num}" '$1 == ln {print $1}' cmpd_${index}))
            #final_sym+=($(awk -v ln="${ln_num}" '$1 == ln {print $2}' cmpd_${index}))
            #final_X+=($(awk -v ln="${ln_num}" '$1 == ln {print $3}' cmpd_${index}))
            #final_Y+=($(awk -v ln="${ln_num}" '$1 == ln {print $4}' cmpd_${index}))
            #final_Z+=($(awk -v ln="${ln_num}" '$1 == ln {print $5}' cmpd_${index}))
        done
    done
